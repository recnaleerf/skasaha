...
func (s *Skasaha) cmdEmoji(ds *discordgo.Session, m *discordgo.MessageCreate, emoji string) error {
	if emoji == "" {
	...
	file := path.Join(s.EmojiDir, name)

	f, err = os.Open(file)
	if err == nil {
		_, err = ds.ChannelFileSend(m.ChannelID, name, f)
	...
	if len(similarNames) == 0 {
		_, err = ds.ChannelMessageSend(m.ChannelID, "Unknown emoji.")
